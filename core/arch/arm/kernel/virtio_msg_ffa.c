...
enum virtio_area_state {
	VIRTIO_AREA_ACTIVE,
	VIRTIO_AREA_UNSHARING,
};
 ...
 * @state:	ACTIVE while mappings are allowed; UNSHARING once unshare has
 *		reserved the area and blocked new mappings.
...
/*
 * Driver registration happens from nex_service_init_late(), before guests
 * exist. Unlike endpoint state, this is one system-wide registry which every
 * VM consults when creating its VM-local bus. Keep it in Nexus memory so a
 * VM does not see its own zero-initialized .bss copy.
 */
static struct virtio_msg_bus_driver *drivers[VIRTIO_MSG_FFA_MAX_DRV] __nex_bss;
static unsigned int drivers_count __nex_bss;
				  ...
				  uint16_t area_id)
{
	...
	if (!a)
		goto out_unlock;
	if (a->state != VIRTIO_AREA_ACTIVE) {
		res = TEE_ERROR_BUSY;
		goto out_unlock;
	}

	/* Keep @a linked and stable while unregistering outside the spinlock. */
	a->state = VIRTIO_AREA_UNSHARING;
	handle = a->handle;
	cpu_spin_unlock_xrestore(&ep->lock, state);

	res = mobj_ffa_unregister_by_cookie(handle);

	state = cpu_spin_lock_xsave(&ep->lock);
	if (!res || res == TEE_ERROR_ITEM_NOT_FOUND)
		TAILQ_REMOVE(&ep->areas, a, link);
	else if (res != TEE_ERROR_BUSY)
		a->state = VIRTIO_AREA_ACTIVE;
	cpu_spin_unlock_xrestore(&ep->lock, state);
				     ...
				     size_t len)
{
	...
	state = cpu_spin_lock_xsave(&ep->lock);
	a = ep_find_area(ep, area_id);
	if (a && a->state == VIRTIO_AREA_ACTIVE) {
		mobj_get(a->mobj);
...
/* Request a yielding notification callback for deferred EVENT_AVAIL work. */
static enum virtio_msg_status
virtio_msg_ffa_schedule_bottom_half(struct virtio_msg_bus *bus)
{
	struct virtio_msg_ffa_ep *ep = bus->ops_cookie;

	if (!notif_async_is_started(ep->ffa_ep_id))
		return VIRTIO_MSG_STATUS_BUSY;

	notif_send_async(NOTIF_VALUE_DO_BOTTOM_HALF, ep->ffa_ep_id);
	return VIRTIO_MSG_STATUS_OK;
}
...
static const struct virtio_msg_bus_ops virtio_msg_ffa_bus_ops = {
	...
	.schedule_bottom_half = virtio_msg_ffa_schedule_bottom_half,
};
...
static enum virtio_msg_status
recv_bus_msg(struct virtio_msg *msg, struct virtio_msg_ffa_ep *ep)
{
	switch (msg->msg_id) {
	...
	default:
		...
		return virtio_msg_recv(msg, VIRTIO_MSG_MAX_SIZE, &ep->bus);
	}

	return VIRTIO_MSG_STATUS_OK;
}

enum virtio_msg_status
virtio_msg_ffa_recv(struct thread_smc_1_2_regs *args, uint16_t caller_id)
{
	...
	enum virtio_msg_status status = VIRTIO_MSG_STATUS_OK;
	...
	if (is_bus && (ffa_bus_id == FFA_BUS_MSG_VERSION ||
		...
		status = recv_bus_msg(msg, ep);
	else
		status = virtio_msg_recv(msg, VIRTIO_MSG_MAX_SIZE, &ep->bus);

	if (status != VIRTIO_MSG_STATUS_OK)
		return status;
...
/* The notification framework invokes this in a yielding VM context. */
				       ...
				       enum notif_event ev)
{
	...
	state = cpu_spin_lock_xsave(&ep_head_lock);
	TAILQ_FOREACH(ep, &ep_head, link) {
		/* Endpoints are never removed, so @ep remains valid unlocked. */
		cpu_spin_unlock_xrestore(&ep_head_lock, state);
		virtio_msg_bus_process_bottom_half(&ep->bus);
		state = cpu_spin_lock_xsave(&ep_head_lock);
	}
	cpu_spin_unlock_xrestore(&ep_head_lock, state);
}

/* Required by notif_register_driver(); no bus memory is touched atomically. */
static void virtio_msg_ffa_atomic_cb(struct notif_driver *ndrv __unused,
				     enum notif_event ev __unused,
				     uint16_t guest_id __unused)
{
}
DECLARE_KEEP_PAGER(virtio_msg_ffa_atomic_cb);

static struct notif_driver virtio_msg_ffa_notif __nex_data = {
	.atomic_cb = virtio_msg_ffa_atomic_cb,
