...
struct spmc_lsp_desc {
	...
	uint32_t properties;
	/*
	 * One or more FF-A UUIDs, each SPMC_WORDS_PER_UUID little-endian
	 * words. An LSP may advertise several UUIDs (e.g. the OP-TEE core
	 * endpoint also carries the virtio-msg bus UUID).
	 */
	uint16_t uuid_count;
...
/*
 * spmc_ffa_set_notification() - set an FF-A notification towards a non-secure
 * endpoint on behalf of the OP-TEE core LSP.
 *
 * Used by the virtio-msg FF-A carrier to deliver VIRTIO_MSG_EVENT_USED as an
 * FF-A notification. Only available when the SPMC is external (S-EL2), where
 * OP-TEE can originate FFA_NOTIFICATION_SET.
 */
