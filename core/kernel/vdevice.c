...
/*
 * This core is a port of the QTEE vhost backend. Driver-provided ring memory
 * is already mapped by the transport, so the QTEE driver-memory copy helpers
 * reduce to memcpy and the release/acquire barriers to dsb(). Feature sets fit
 * in 64 bits, so a plain uint64_t replaces the QTEE virtio_features_t bitset.
 */
...
bool vdevice_vq_try_get(struct vdevice_vq *vq)
{
	uint32_t exceptions = cpu_spin_lock_xsave(&vq->lock);
	bool ret = false;

	if (vq->ready && !vq->quiescing)
		ret = refcount_inc(&vq->refs);
...
void vdevice_vq_put(struct vdevice_vq *vq)
{
	uint32_t exceptions = cpu_spin_lock_xsave(&vq->lock);

	assert(refcount_val(&vq->refs));
	refcount_dec(&vq->refs);

	cpu_spin_unlock_xrestore(&vq->lock, exceptions);
}

bool vdevice_vq_is_ready(struct vdevice_vq *vq)
{
	uint32_t exceptions = cpu_spin_lock_xsave(&vq->lock);
	bool ret = vq->ready && !vq->quiescing;
...
/*
 * Split-ring accessors. The ring pointers refer to driver-provided shared
 * memory mapped by the transport; these wrap the accesses so ordering and the
 * used_event / avail_event trailing fields live in one place.
 */
...
/* Queue reset and device status transitions */
/* Reset all per-queue state (see vdevice_reset_queue for the device-type part) */
static void vdevice_vq_reset_locked(struct vdevice_vq *vq)
{
	assert(!refcount_val(&vq->refs));
	vq->ready = false;
	vq->quiescing = false;
...
/* Block new queue users. Returns true once all existing users have exited. */
static bool vdevice_vq_quiesce_locked(struct vdevice_vq *vq)
{
	if (!vq->quiescing) {
		vq->quiescing = true;
		vq->ready = false;
		if (refcount_val(&vq->refs))
			refcount_dec(&vq->refs); /* Drop the enabled-queue reference */
	}

	return !refcount_val(&vq->refs);
}

/* Reset callers must quiesce all queues before calling this function. */
...
static bool vdevice_quiesce(struct vdevice *vdev)
{
	int n = 0;
	bool idle = true;

	for (n = 0; n < vdev->num_queues; n++) {
		struct vdevice_vq *vq = &vdev->vqs[n];
		uint32_t exceptions = cpu_spin_lock_xsave(&vq->lock);

		if (!vdevice_vq_quiesce_locked(vq))
			idle = false;
		cpu_spin_unlock_xrestore(&vq->lock, exceptions);
	}
...
static int vdevice_reset(struct vdevice *vdev)
{
	int n = 0;

	if (!vdevice_quiesce(vdev))
		return VDEVICE_ERR_BUSY;
		     ...
		     uint64_t used_ba, uint16_t num)
{
	...
	/* Do not (re)program a running or quiescing queue. */
	uint32_t exceptions = cpu_spin_lock_xsave(&vq->lock);

	if (vq->ready || vq->quiescing || refcount_val(&vq->refs)) {
		cpu_spin_unlock_xrestore(&vq->lock, exceptions);
		return -1;
	}
	...
	/* Seed index and cache state from the driver-provided rings */
	dsb();
...
void vdevice_set_ring_ready(struct vdevice_vq *vq, bool ready)
{
	...
	if (ready) {
		/* A zero-size or quiescing queue cannot be enabled. */
		if (vq->num && !vq->quiescing && !vq->ready &&
		    !refcount_val(&vq->refs)) {
			refcount_set(&vq->refs, 1);
			vq->ready = true;
		}
	} else {
		vdevice_vq_quiesce_locked(vq);
	}
...
int vdevice_reset_queue(struct vdevice *vdev, int qid)
{
	...
	idle = vdevice_vq_quiesce_locked(vq);
	cpu_spin_unlock_xrestore(&vq->lock, exceptions);
	if (!idle)
		return VDEVICE_ERR_BUSY;
...
static void *vdevice_map_driver_memory(struct vdevice_vq *vq,
				       uint64_t dma_addr, size_t size)
{
	return vq->vdev->dma.map(vq->vdev, dma_addr, size);
}

static void vdevice_unmap_driver_memory(struct vdevice_vq *vq, void *addr,
					 size_t size)
{
	vq->vdev->dma.unmap(vq->vdev, addr, size);
}
			       ...
			       uint16_t *out, uint16_t *in, uint16_t *sgs_idx)
{
	...
	for (;;) {
		...
		if (desc.flags & VIRTQ_DESC_F_INDIRECT) {
			...
			tbl = vdevice_map_driver_memory(vq, desc.addr, desc.len);
			if (!tbl)
				return -1;
			...
			res = vdevice_chain_accum(vq, 0, tbl, nent, out, in,
						  sgs_idx);
			vdevice_unmap_driver_memory(vq, tbl, desc.len);
...
int vdevice_get_vq_desc(struct vdevice_vq *vq, struct vdevice_chain *chain)
{
	...
	if (!vdevice_vq_is_ready(vq) || !vq->desc || !vq->avail)
		return -1;
	...
	res = vdevice_chain_get_desc(vq, NULL, vq->num, head, &desc);
	if (res)
		return res;
	if (desc.flags & VIRTQ_DESC_F_INDIRECT) {
		struct virtq_desc *tbl = NULL;
...
/* Used-ring completion and notification control */
...
/* Copy @len bytes from a driver DMA address into a device buffer */
static int vdevice_copy_from_driver_dma(struct vdevice_vq *vq, void *dst,
					uint64_t src_dma, size_t len)
{
	...
	ptr = vdevice_map_driver_memory(vq, src_dma, len);
	if (!ptr)
		return -1;

	memcpy(dst, ptr, len);
	vdevice_unmap_driver_memory(vq, ptr, len);

	return 0;
}

/* Copy @len bytes from a device buffer into a driver DMA address */
static int vdevice_copy_to_driver_dma(struct vdevice_vq *vq,
				      uint64_t dst_dma, const void *src, size_t len)
{
	void *ptr = NULL;
	...
	ptr = vdevice_map_driver_memory(vq, dst_dma, len);
	if (!ptr)
		return -1;
			       ...
			       size_t len, bool to)
{
	...
	while (done < len && seg < sgl->num) {
		...
		if (to) {
			if (vdevice_copy_to_driver_dma(vq, s->addr + off,
						      lin + done, chunk))
				return -1;
		} else {
			if (vdevice_copy_from_driver_dma(vq, lin + done,
							s->addr + off, chunk))
				return -1;
		}
