			 ...
			 const struct virtio_msg_bus_ops *ops, void *cookie)
{
	...
	bus->pending_lock = SPINLOCK_UNLOCK;
	TAILQ_INIT(&bus->pending_vqs);
}

/*
 * Driver-memory accessor handed to the device core. The core passes back the
				 ...
				 struct virtio_msg_bus_driver *drv)
{
	...
	if (vmdev->vdev.num_queues) {
		vmdev->pending_vqs = calloc(vmdev->vdev.num_queues,
					    sizeof(*vmdev->pending_vqs));
		if (!vmdev->pending_vqs) {
			if (drv->deinit)
				drv->deinit(&vmdev->vdev);
			bus->devs[vmdev->dev_id] = NULL;
			free(vmdev);
			return -1;
		}
...
handle_set_device_status(struct virtio_msg *msg, struct virtio_msg_dev *vmdev)
{
	 ...
	 */
	if (vdevice_status_write(&vmdev->vdev, v->status) == VDEVICE_ERR_BUSY)
...
static enum virtio_msg_status
handle_reset_vqueue(struct virtio_msg *msg, struct virtio_msg_dev *vmdev)
{
	...
	if (req->index < (uint32_t)vdev->num_queues &&
	    vdevice_reset_queue(vdev, req->index) == VDEVICE_ERR_BUSY)
		return VIRTIO_MSG_STATUS_BUSY;
	msg->msg_size = sizeof(*msg);
...
static enum virtio_msg_status
queue_event_avail(struct virtio_msg_dev *vmdev, struct vdevice_vq *vq)
{
	struct virtio_msg_bus *bus = vmdev->bus;
	struct virtio_msg_pending_vq *pending = NULL;
	uint32_t state = 0;
	enum virtio_msg_status status = VIRTIO_MSG_STATUS_OK;
	bool added = false;

	if (!bus->ops->schedule_bottom_half || vq->qid < 0 ||
	    vq->qid >= vmdev->vdev.num_queues)
		return VIRTIO_MSG_STATUS_BUSY;

	pending = &vmdev->pending_vqs[vq->qid];
	state = cpu_spin_lock_xsave(&bus->pending_lock);
	if (!pending->queued) {
		pending->vq = vq;
		pending->queued = true;
		TAILQ_INSERT_TAIL(&bus->pending_vqs, pending, link);
		added = true;
	}
	cpu_spin_unlock_xrestore(&bus->pending_lock, state);

	status = bus->ops->schedule_bottom_half(bus);
	if (status == VIRTIO_MSG_STATUS_OK)
		return status;

	/* Do not acknowledge an event that cannot be delivered later. */
	state = cpu_spin_lock_xsave(&bus->pending_lock);
	if (added && pending->queued) {
		TAILQ_REMOVE(&bus->pending_vqs, pending, link);
...
void virtio_msg_bus_process_bottom_half(struct virtio_msg_bus *bus)
{
	for (;;) {
		struct virtio_msg_pending_vq *pending = NULL;
		struct vdevice_vq *vq = NULL;
		uint32_t state = cpu_spin_lock_xsave(&bus->pending_lock);

		pending = TAILQ_FIRST(&bus->pending_vqs);
		if (!pending) {
			cpu_spin_unlock_xrestore(&bus->pending_lock, state);
			return;
		}
		...
		if (vq->notify && vdevice_vq_try_get(vq)) {
			vq->notify(vq->vdev, vq);
			vdevice_vq_put(vq);
		}
	}
}
...
static enum virtio_msg_status
handle_event_avail(struct virtio_msg *msg, struct virtio_msg_dev *vmdev)
{
	...
	if (ev->index < (uint32_t)vdev->num_queues) {
		...
		if (vq->notify && vdevice_vq_is_ready(vq)) {
			enum virtio_msg_status status = queue_event_avail(vmdev, vq);
...
static enum virtio_msg_status
virtio_msg_recv_transport(struct virtio_msg *msg, struct virtio_msg_bus *bus)
{
	...
	case VIRTIO_MSG_SET_DEVICE_STATUS:
		return handle_set_device_status(msg, vmdev);
	...
	case VIRTIO_MSG_RESET_VQUEUE:
		return handle_reset_vqueue(msg, vmdev);
	case VIRTIO_MSG_EVENT_AVAIL:
		return handle_event_avail(msg, vmdev);
	default:
...
enum virtio_msg_status virtio_msg_recv(struct virtio_msg *msg, size_t max_size,
				       struct virtio_msg_bus *bus)
{
	...
	if (is_bus)
		...
		status = virtio_msg_recv_transport(msg, bus);
