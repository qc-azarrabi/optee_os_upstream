...
/* Queue is quiescing until active users complete; caller must retry. */
#define VDEVICE_ERR_BUSY		-2
...
/*
 * struct vdevice_dma - per-device accessor to driver memory.
 ...
 *			enabled.
 * @quiescing:		true once reset/stop has blocked new users of the queue.
 * @refs:		one base reference while enabled, plus one for each active
 *			queue user. This prevents reset from invalidating ring state
 *			while a device callback is processing it.
 * @lock:		protects queue state and @refs. Ring users hold a temporary
 *			reference instead of this spinlock while processing descriptors.
 ...
 * transition, or VDEVICE_ERR_BUSY while queue users are draining.
 */
 ...
 *
 * Must be called at FEATURES_OK. Seeds the index/cache state from the driver
 * rings and latches the negotiated per-queue feature bits. The queue is not
 * consumed until it is marked ready with vdevice_set_ring_ready(). The queue
 * must not be quiescing or pinned by an active user.
 */
...
/*
 * vdevice_vq_try_get() - pin an enabled queue while accessing its ring state.
 *
 * Returns false if the queue is disabled or quiescing. Each successful call
 * must be paired with vdevice_vq_put(). The caller must not hold vq->lock.
 */
bool vdevice_vq_try_get(struct vdevice_vq *vq);
void vdevice_vq_put(struct vdevice_vq *vq);

/* Return whether a queue can accept a new user. */
 ...
 * the ring is empty, or a negative value on a malformed chain. The caller
 * must hold a reference obtained with vdevice_vq_try_get().
 */
