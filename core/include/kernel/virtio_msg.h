...
struct virtio_msg_pending_vq;

TAILQ_HEAD(virtio_msg_pending_vq_head, virtio_msg_pending_vq);
...
struct virtio_msg_bus_ops {
	...
	enum virtio_msg_status (*schedule_bottom_half)(
...
struct virtio_msg_bus {
	...
	void *ops_cookie;
	unsigned int pending_lock;
	struct virtio_msg_pending_vq_head pending_vqs;
};

/* One preallocated pending-list node for each queue of a bus device. */
struct virtio_msg_pending_vq {
	struct vdevice_vq *vq;
	bool queued;
	TAILQ_ENTRY(virtio_msg_pending_vq) link;
};
...
struct virtio_msg_dev {
	...
	uint16_t dev_id;
	struct virtio_msg_bus *bus;
	struct virtio_msg_pending_vq *pending_vqs;
};
 ...
 * message: it returns a null/error response instead. BUSY means that an
 * event could not be accepted and must be retried by the carrier.
 */
enum virtio_msg_status virtio_msg_recv(struct virtio_msg *msg, size_t max_size,
				       struct virtio_msg_bus *bus);

/* Drain deferred EVENT_AVAIL callbacks in a yielding context. */
